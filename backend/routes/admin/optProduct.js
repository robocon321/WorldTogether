...
router.get('/', verifyToken, async (req, res) => {
  ...
  try {
    const optProductTitle = await OptProductTitle.find({...query_title});
    optProduct.product_titles = optProductTitle;

    optProductTitle.forEach(async item => {
      const optProductValue = await OptProductValue.find({...query_value, opt_title_id: item._id});
