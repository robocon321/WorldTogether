import { ACTIONS } from "../../../actions/admin/product/ProductEditAction"
...
const reducer = (state = init, action) => {
  switch(action.type) {
    case ACTIONS.LOAD_OPTION_PRODUCT:
      const codes = [];
      action.product_values.forEach(item => {
        const i = codes.indexOf(item.code);
        const option = {
          product_title: action.product_titles.find(titleItem => titleItem._id === item.opt_title_id),
          product_value: item
        };

        if(!codes.includes(item.code)) {
          codes.push(item.code);
          state.group.push([option]);
        } else {
          state.group[i].push(option);
        }
      });
      break;
    
    case ACTIONS.LOAD_ATTRIBUTE_VALUE:
      state.attributeValues = action.values;
      break;
    ...
    case ACTIONS.LOAD_ATTRIBUTE:
      ...
      action.attributes.forEach(item => {
        state.attributeValues.push({attr_id: item._id, text_value: '', number_value: 0, date_value: new Date()});
      })
    ...
    case ACTIONS.CHANGE_ATTRIBUTE:
      ...
      const attrValue = attributeValues.find(item => item.attr_id === action.attr_id);
      attrValue[action.name] = action.value;
      break;
